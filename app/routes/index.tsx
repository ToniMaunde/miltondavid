    ...
    <>
      ...
      <main className="flex flex-col">
        <section className="px-4 pt-10 pb-16">
          <h1 className="text-white font-bold text-3xl tracking-tight">Scracthing my itch for creating things</h1>
          <p className="text-light-gray mt-1 mb-10">and while doing so, I try to improve people&#39;s lives with my creations.</p>
          <Link to="/projects" className="px-2 py-3 bg-primary rounded text-bg-darker font-semibold text-lg tracking-[0.01em]">See Projects</Link>
        </section>
        <section className="px-4 py-10 bg-white">
          <h2 className="text-bg text-xl font-bold mb-2">A little about me</h2>
        ...
        <section className="px-4 py-10">
          <h2 className="text-white text-xl font-bold mb-2">What else?</h2>
          <p className="text-light-gray font-medium mb-2">Well, I guess that sums it up. If you have any other pertinent questions, <Link to="/contact" className="underline">contact me</Link>. I&#39;ll get back to you as soon as I can. And down below, you can find links to some of my profiles on the Internet.</p>
          <ul className="grid grid-cols-3">
            {
              links.map(({ icon, link }) => (
                <li key={link} className="first:mt-4 even:mt-10">
                  <a href={link} target="_blank" rel="noreferrer">
                    <Icon {...icon} customClasses="fill-white "/> 
                  </a>
                </li>
              ))
            }
